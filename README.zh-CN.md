...
![sub2api 上游状态面板移动端展示图](docs/images/dashboard-preview-mobile.png)
...
- 在 `7 天` 用量进度条内叠加显示当前用量（实色）、推荐用量（浅绿色）和窗口已过占比（内缩圆点）
...
- `ENABLE_ANNOUNCEMENTS`：是否显示 sub2api 最新的有效公告，默认 `true`
...
`7 天` 窗口只使用一条进度条：实色表示当前用量，浅绿色表示推荐用量，内缩圆点表示窗口已过时间占比。`5 小时` 进度条不显示圆点，以降低短窗口信息的视觉密度。当前用量会根据消耗程度显示为绿、橙或红色，并覆盖下方的推荐用量层。
